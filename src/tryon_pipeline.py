...
class StableDiffusionXLInpaintPipeline(
    ...
    def encode_prompt(
        ...
        # duplicate text embeddings for each generation per prompt, using cpu friendly method
        prompt_embeds = prompt_embeds.repeat(1, num_images_per_prompt, 1)
        prompt_embeds = prompt_embeds.view(bs_embed * num_images_per_prompt, seq_len, -1)

        if do_classifier_free_guidance:
            # duplicate unconditional embeddings for each generation per prompt, using cpu friendly method
    ...
    def prepare_mask_latents(
        ...
        # duplicate mask and masked_image_latents for each generation per prompt, using cpu friendly method
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not output_type == "latent":
            # make sure the VAE is in float32 mode, as it overflows in float32
