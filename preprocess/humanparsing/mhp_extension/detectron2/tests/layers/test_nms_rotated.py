...
class TestNMSRotated(unittest.TestCase):
    ...
    def test_batched_nms_rotated_0_degree_cuda(self):
        ...
        for iou in [0.2, 0.5, 0.8]:
            backup = boxes.clone()
            keep_ref = batched_nms(boxes.to(device), scores.to(device), idxs, iou)
            self.assertTrue(torch.allclose(boxes, backup), "boxes modified by batched_nms")
            backup = rotated_boxes.clone()
            keep = batched_nms_rotated(rotated_boxes.to(device), scores.to(device), idxs, iou)
            self.assertTrue(
    ...
    def test_nms_rotated_0_degree_cuda(self):
        ...
        for iou in [0.2, 0.5, 0.8]:
            ...
            r_cuda = nms_rotated(rotated_boxes.to(device), scores.to(device), iou)
